      ...
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
        <form @submit.prevent="handleSubmit">
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4">코드 그룹 수정</h3>

            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-700">코드 그룹명</label>
                <input
                  v-model="form.name"
                  type="text"
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700">코드 그룹 ID</label>
                <input
                  v-model="form.codeGroupId"
                  type="text"
                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500"
                  required
                  pattern="[A-Za-z0-9_]+"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    watch(() => props.codeGroup, (newValue) => {
      if (newValue) {
        form.value = {
          name: newValue.name,
          codeGroupId: newValue.codeGroupId,
